...
export class UpdateContactTool {
  ...
  async execute(sessionContext, input) {
    ...
    if (fieldsToUpdate.length === 0) {
      return { success: false, error: "'fields' must contain at least one property to update" };
    }

    // 2. Convert contact_id to number
    const contactId = Number(contact_id);
    if (isNaN(contactId) || contactId <= 0) {
      return { success: false, error: "'contact_id' must be a positive number" };
    }
    ...
    try {
      ...
      return { success: false, error: `Failed to retrieve contact: ${error.message}` };
    }

    if (!contact) {
      return { success: false, error: `Contact not found (ID: ${contactId})` };
    }
